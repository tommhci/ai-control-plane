...
## Global canon pointer (ai-control-plane, G/L layering)

This repo is attached to an AI control plane — its repo root is recorded in
`.control-plane/adapter.json` (`controlPlaneRepo` field). The control plane's
cross-tool lessons registry (`docs/cross-tool-lessons.md` there) is the
G-layer canon: read it before governed or destructive work (L1 governance
commit sequence, L2 history-before-repair, U4 pre-deletion gate). Pointer
direction is one-way: this client points up to the canon; the canon never
references this client's internals. Project-specific lessons stay in this
repo (L layer).
