...
8b. **Multi-agent ROI log (mandatory when applicable):** if this session
    used a multi-agent pattern (fan-out, paired audit, orchestrator), append
    one entry to `.agents/MULTI_AGENT_ROI_LOG.md` per the comparison rubric
    (task class, mode, model family per role, confounds, verdict
    helped/hurt/neutral). If none was used and none was warranted, write
    `multi_agent_use: none`. Missing entry = session not closed (2026-08-29).
